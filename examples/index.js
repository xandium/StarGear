...
const cache = new RainCache({
  storage: { default: new RedisEngine({ host: "localhost" }) },
  debug: false
});
...
bot.on("messageCreate", async msg => {
  ...
  console.log(
    `${channel.name}: ${msg.author.username}#${msg.author.discriminator}: ${
      msg.content
    }`
  );
  if (msg.content === "example ping") {
    ...
    return bot.rest.channel.editMessage(
      msg.channel_id,
      pingMsg.id,
      `pong \`${Date.now() - time}ms\``
    );
  }
});
let init = async () => {
